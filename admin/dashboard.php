...
try {
    ...
    $sql_reservations = "SELECT COUNT(*) as total_reservations, 
                         ...
                         AND (CASE 
                              ...
                              ELSE 'Standard'
                              END) = f.trip_type
                         WHERE r.payment_status != 'Annulé' AND r.payment_status != ''";
...
// Déterminer la période pour le rapport périodique global
    ...
    if ($report_period === 'daily') {
        $sql_periodic_reservations = "SELECT r.passenger_name, f.tariff, f.port_fee, f.tax, r.total_amount, r.payment_status
                                      ...
                                      AND (CASE 
                                           ...
                                           ELSE 'Standard'
                                           END) = f.trip_type
                                      WHERE r.departure_date = ? AND r.payment_status != 'Annulé' AND r.payment_status != ''";
        $stmt_periodic_reservations = $pdo->prepare($sql_periodic_reservations);
        $stmt_periodic_reservations->execute([$report_date]);
        ...
        $sql_periodic_reservations = "SELECT r.passenger_name, f.tariff, f.port_fee, f.tax, r.total_amount, r.payment_status
                                      ...
                                      AND (CASE 
                                           ...
                                           ELSE 'Standard'
                                           END) = f.trip_type
                                      WHERE DATE_FORMAT(r.departure_date, '%Y-%m') = ? AND r.payment_status != 'Annulé' AND r.payment_status != ''";
        $stmt_periodic_reservations = $pdo->prepare($sql_periodic_reservations);
        $stmt_periodic_reservations->execute([$report_month]);
        ...
        $sql_periodic_reservations = "SELECT r.passenger_name, f.tariff, f.port_fee, f.tax, r.total_amount, r.payment_status
                                      ...
                                      AND (CASE 
                                           ...
                                           ELSE 'Standard'
                                           END) = f.trip_type
                                      WHERE YEAR(r.departure_date) = ? AND r.payment_status != 'Annulé' AND r.payment_status != ''";
        $stmt_periodic_reservations = $pdo->prepare($sql_periodic_reservations);
        $stmt_periodic_reservations->execute([$report_year]);
...
try {
    if ($report_period === 'daily') {
        ...
        $stmt_periodic_colis->execute([$report_date]);
    } elseif ($report_period === 'monthly') {
        $sql_periodic_colis = "SELECT sender_name, package_reference, package_fee
                               FROM colis 
                               WHERE DATE_FORMAT(created_at, '%Y-%m') = ?";
        $stmt_periodic_colis = $pdo->prepare($sql_periodic_colis);
        $stmt_periodic_colis->execute([$report_month]);
    } else { // yearly
        $sql_periodic_colis = "SELECT sender_name, package_reference, package_fee
                               FROM colis 
                               WHERE YEAR(created_at) = ?";
        $stmt_periodic_colis = $pdo->prepare($sql_periodic_colis);
        $stmt_periodic_colis->execute([$report_year]);
    }
...
try {
    if ($report_period === 'daily') {
        ...
        $stmt_periodic_totals->execute([$report_date]);
    } elseif ($report_period === 'monthly') {
        $sql_periodic_totals = "SELECT COALESCE(SUM(f.tariff), 0) as periodic_tariff,
                                       COALESCE(SUM(f.tax), 0) as periodic_tax,
                                       COALESCE(SUM(f.port_fee), 0) as periodic_port_fee,
                                ...
                                WHERE DATE_FORMAT(r.departure_date, '%Y-%m') = ? AND r.payment_status != 'Annulé' AND r.payment_status != ''";
        $stmt_periodic_totals = $pdo->prepare($sql_periodic_totals);
        $stmt_periodic_totals->execute([$report_month]);
    } else { // yearly
        $sql_periodic_totals = "SELECT COALESCE(SUM(f.tariff), 0) as periodic_tariff,
                                       COALESCE(SUM(f.tax), 0) as periodic_tax,
                                       COALESCE(SUM(f.port_fee), 0) as periodic_port_fee,
                                ...
                                WHERE YEAR(r.departure_date) = ? AND r.payment_status != 'Annulé' AND r.payment_status != ''";
        $stmt_periodic_totals = $pdo->prepare($sql_periodic_totals);
        $stmt_periodic_totals->execute([$report_year]);
    }
    ...
    $sql_periodic_colis_totals = "SELECT COALESCE(SUM(package_fee), 0) as periodic_package_fee
                                  FROM colis 
                                  WHERE " . ($report_period === 'daily' ? "DATE(created_at) = ?" : ($report_period === 'monthly' ? "DATE_FORMAT(created_at, '%Y-%m') = ?" : "YEAR(created_at) = ?"));
    $stmt_periodic_colis_totals = $pdo->prepare($sql_periodic_colis_totals);
    $stmt_periodic_colis_totals->execute([$report_period === 'daily' ? $report_date : ($report_period === 'monthly' ? $report_month : $report_year)]);
...
// Préparer les données pour le graphique et l'exportation PDF global
$report_data = [
    'title' => $report_title,
...
<body>
    ...
    <main class="main-content">
        <div class="overlay">
                        ...
                        <tbody>
                            <?php foreach ($periodic_reservations as $reservation): ?>
                                <tr>
                                    ...
                                    <td><?php echo number_format($reservation['tariff'] ?? 0, 2); ?></td>
                                    <td><?php echo number_format($reservation['port_fee'] ?? 0, 2); ?></td>
                                    <td><?php echo number_format($reservation['tax'] ?? 0, 2); ?></td>
            ...
            <div class="user-report-section">
                <h2>Rapport par Utilisateur</h2>
                <form id="user-report-form" class="user-report-filter">
                    ...
                    <label for="user_report_period">Période : </label>
                    <select name="user_report_period" id="user_report_period">
                        <option value="daily">Journalier</option>
                        <option value="monthly">Mensuel</option>
                        <option value="yearly">Annuel</option>
                    </select>
                    <div id="user-daily-input">
                        <label for="user_report_date">Date : </label>
                        <input type="date" id="user_report_date" name="user_report_date" value="<?php echo date('Y-m-d'); ?>">
                    </div>
                    <div id="user-monthly-input" style="display: none;">
                        <label for="user_report_month">Mois : </label>
                        <input type="month" id="user_report_month" name="user_report_month" value="<?php echo date('Y-m'); ?>">
                    </div>
                    <div id="user-yearly-input" style="display: none;">
                        <label for="user_report_year">Année : </label>
                        <input type="number" id="user_report_year" name="user_report_year" value="<?php echo date('Y'); ?>" min="2000" max="<?php echo date('Y'); ?>">
                    </div>
                    <button type="button" onclick="fetchUserReport()">Afficher</button>
                </form>
                <div id="user-report-container" style="display: none;">
                    <h3 id="user-report-title">Rapport</h3>
                    <button onclick="exportUserReportPDF()" class="export-btn"><i class="fas fa-file-pdf"></i> Exporter en PDF</button>
                    <div class="daily-summary">
                        <h4>Totaux Financiers</h4>
                        <table id="user-totals-table">
                            <thead>
                                <tr>
                                    <th>Total Tarifs (KMF)</th>
                                    <th>Total Taxes (KMF)</th>
    ...
    <script>
        ...
        // Données pour l'exportation PDF global
