...
// Supprimer une réservation (supprime les deux enregistrements pour aller-retour)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_reservation'])) {
    $id = (int)$_POST['delete_reservation'];
    try {
        $pdo->beginTransaction();
        // Récupérer la réservation et le reference_id
        $stmt = $pdo->prepare("SELECT reference_id, ticket_id, trip_id, seat_number, direction FROM reservations WHERE id = ? AND user_id = ?");
        $stmt->execute([$id, $_SESSION['user_id']]);
        $reservation = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($reservation) {
            $reference_id = $reservation['reference_id'];
            // Supprimer toutes les réservations avec le même reference_id
            $stmt = $pdo->prepare("SELECT ticket_id, trip_id, seat_number FROM reservations WHERE reference_id = ? AND user_id = ?");
            $stmt->execute([$reference_id, $_SESSION['user_id']]);
            $reservations_to_delete = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($reservations_to_delete as $res) {
                // Restaurer le ticket
                $pdo->prepare("UPDATE tickets SET status = 'available' WHERE id = ?")->execute([$res['ticket_id']]);
                // Restaurer la place
                $pdo->prepare("UPDATE seats SET status = 'free' WHERE trip_id = ? AND seat_number = ?")
                    ->execute([$res['trip_id'], $res['seat_number']]);
            }
            // Supprimer les réservations
            $pdo->prepare("DELETE FROM reservations WHERE reference_id = ? AND user_id = ?")->execute([$reference_id, $_SESSION['user_id']]);
            // Restaurer la référence
            $pdo->prepare("UPDATE reference SET status = 'available' WHERE id = ?")->execute([$reference_id]);
            $pdo->commit();
            $_SESSION['success'] = "Réservation(s) supprimée(s) avec succès.";
        } else {
...
// Annuler une réservation (annule les deux enregistrements pour aller-retour)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_reservation'])) {
    $id = (int)$_POST['cancel_reservation'];
    try {
        $stmt = $pdo->prepare("SELECT reference_id FROM reservations WHERE id = ? AND user_id = ?");
        $stmt->execute([$id, $_SESSION['user_id']]);
        $reservation = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($reservation) {
            $reference_id = $reservation['reference_id'];
            $pdo->beginTransaction();
            // Récupérer toutes les réservations avec le même reference_id
            $stmt = $pdo->prepare("SELECT ticket_id, trip_id, seat_number FROM reservations WHERE reference_id = ? AND user_id = ?");
            $stmt->execute([$reference_id, $_SESSION['user_id']]);
            $reservations_to_cancel = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($reservations_to_cancel as $res) {
                // Restaurer le ticket
                $pdo->prepare("UPDATE tickets SET status = 'available' WHERE id = ?")->execute([$res['ticket_id']]);
                // Restaurer la place
                $pdo->prepare("UPDATE seats SET status = 'free' WHERE trip_id = ? AND seat_number = ?")
            ...
            $stmt = $pdo->prepare("UPDATE reservations SET payment_status = 'Annulé' WHERE reference_id = ? AND user_id = ?");
            $stmt->execute([$reference_id, $_SESSION['user_id']]);
            // Restaurer la référence
            $pdo->prepare("UPDATE reference SET status = 'available' WHERE id = ?")->execute([$reference_id]);
            $pdo->commit();
            $_SESSION['success'] = "Réservation(s) annulée(s) avec succès.";
        } else {
            $_SESSION['error'] = "Réservation non trouvée ou non autorisée.";
        }
    } catch (PDOException $e) {
...
// Récupérer les réservations de l'utilisateur, regroupées par reference_id
try {
    $sql = "SELECT r.id, r.passenger_name, r.phone_number, r.nationality, r.passenger_type, r.with_baby, r.departure_date,
                   ...
                   r.modification, r.penalty, r.direction, r.tariff, r.port_fee, r.tax,
            ...
            ORDER BY ref.reference_number, r.direction DESC, r.departure_date DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
    $reservations = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Regrouper les réservations par reference_id
    $grouped_reservations = [];
    foreach ($reservations as $reservation) {
        $ref_id = $reservation['reference_id'];
        if (!isset($grouped_reservations[$ref_id])) {
            $grouped_reservations[$ref_id] = [
                'aller' => null,
                'retour' => null,
                'passenger_info' => [
                    ...
                    'with_baby' => $reservation['with_baby'],
                    'payment_status' => $reservation['payment_status'],
                    'modification' => $reservation['modification'],
                    'penalty' => $reservation['penalty'],
                    'reference_number' => $reservation['reference_number']
                ],
                'total_amount' => $reservation['total_amount'],
                'tariff' => $reservation['tariff'],
                'port_fee' => $reservation['port_fee'],
                'tax' => $reservation['tax']
            ];
        }
        $direction = $reservation['direction'];
        $grouped_reservations[$ref_id][$direction === 'Aller' ? 'aller' : 'retour'] = $reservation;
    }
} catch (PDOException $e) {
...
<head>
    ...
    <style>
        ...
        .section-2a, .section-2b, .section-3a, .section-3b, .section-4a, .section-5a {
            font-size: 9px;
            padding: 3px;
        }
        .section-2a {
        ...
        @media print {
            ...
            .section-2a, .section-2b, .section-3a, .section-3b, .section-4a, .section-4b, .section-5a {
                font-size: 9px;
                padding: 1mm;
            }
            .section p {
        ...
        @media (max-width: 600px) {
            ...
            .section-1a, .section-1b, .section-2a, .section-2b, .section-3a, .section-3b, .section-4a, .section-4b, .section-5a, .section-6 {
                grid-column: 1 / 2;
            }
            ...
            .section-4b { grid-row: 8 / 9; }
            .section-5a { grid-row: 9 / 10; }
            .section-6 { grid-row: 10 / 11; }
        }
    </style>
</head>
<body>
    ...
    <main class="main-content">
        <div class="overlay">
            ...
            <div class="reservations-list-container">
                ...
                <?php if (!empty($grouped_reservations)): ?>
                    <table>
                        <thead>
                            <tr>
                                <th>Passager</th>
                        ...
                        <tbody>
                            <?php foreach ($grouped_reservations as $ref_id => $group): ?>
                                <?php
                                $aller = $group['aller'];
                                $retour = $group['retour'];
                                $main_reservation = $aller ?? $retour; // Utiliser aller ou retour comme référence principale
                                ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['passenger_name']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['phone_number']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['nationality']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['passenger_type']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['with_baby']); ?></td>
                                    <td><?php echo htmlspecialchars($aller ? 'Aller' : 'Retour'); ?></td>
                                    <td><?php echo htmlspecialchars($main_reservation['departure_date'] . ' ' . $main_reservation['departure_time']); ?></td>
                                    <td><?php echo htmlspecialchars($main_reservation['departure_port'] . ' → ' . $main_reservation['arrival_port']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['reference_number']); ?></td>
                                    <td><?php echo htmlspecialchars($main_reservation['ticket_number']); ?></td>
                                    <td><?php echo htmlspecialchars($main_reservation['ticket_type']); ?></td>
                                    <td><?php echo htmlspecialchars($main_reservation['seat_number']); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['payment_status']); ?></td>
                                    <td><?php echo htmlspecialchars(number_format($group['tariff'], 2)); ?></td>
                                    <td><?php echo htmlspecialchars(number_format($group['port_fee'], 2)); ?></td>
                                    <td><?php echo htmlspecialchars(number_format($group['tax'], 2)); ?></td>
                                    <td><?php echo htmlspecialchars(number_format($group['total_amount'], 2)); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['modification'] ?? 'Aucune'); ?></td>
                                    <td><?php echo htmlspecialchars($group['passenger_info']['penalty'] ?? 'Aucune'); ?></td>
                                    <td>
                                        <button class="btn view-ticket-btn"
                                                data-id="<?php echo $main_reservation['id']; ?>"
                                                data-passenger-name="<?php echo htmlspecialchars($group['passenger_info']['passenger_name']); ?>"
                                                data-phone-number="<?php echo htmlspecialchars($group['passenger_info']['phone_number']); ?>"
                                                data-nationality="<?php echo htmlspecialchars($group['passenger_info']['nationality']); ?>"
                                                data-passenger-type="<?php echo htmlspecialchars($group['passenger_info']['passenger_type']); ?>"
                                                data-with-baby="<?php echo htmlspecialchars($group['passenger_info']['with_baby']); ?>"
                                                data-departure-date="<?php echo htmlspecialchars($main_reservation['departure_date']); ?>"
                                                data-departure-time="<?php echo htmlspecialchars($main_reservation['departure_time']); ?>"
                                                data-departure-port="<?php echo htmlspecialchars($main_reservation['departure_port']); ?>"
                                                data-arrival-port="<?php echo htmlspecialchars($main_reservation['arrival_port']); ?>"
                                                data-reference-number="<?php echo htmlspecialchars($group['passenger_info']['reference_number']); ?>"
                                                data-ticket-number="<?php echo htmlspecialchars($main_reservation['ticket_number']); ?>"
                                                data-ticket-type="<?php echo htmlspecialchars($main_reservation['ticket_type']); ?>"
                                                data-seat-number="<?php echo htmlspecialchars($main_reservation['seat_number']); ?>"
                                                data-tariff="<?php echo number_format($group['tariff'], 2); ?>"
                                                data-port-fee="<?php echo number_format($group['port_fee'], 2); ?>"
                                                data-tax="<?php echo number_format($group['tax'], 2); ?>"
                                                data-total-amount="<?php echo number_format($group['total_amount'], 2); ?>"
                                                data-modification="<?php echo htmlspecialchars($group['passenger_info']['modification'] ?? 'Aucune'); ?>"
                                                data-penalty="<?php echo htmlspecialchars($group['passenger_info']['penalty'] ?? 'Aucune'); ?>"
                                                data-direction="<?php echo htmlspecialchars($main_reservation['direction']); ?>"
                                                <?php if ($retour): ?>
                                                    data-retour-id="<?php echo $retour['id']; ?>"
                                                    data-retour-departure-date="<?php echo htmlspecialchars($retour['departure_date']); ?>"
                                                    data-retour-departure-time="<?php echo htmlspecialchars($retour['departure_time']); ?>"
                                                    data-retour-departure-port="<?php echo htmlspecialchars($retour['departure_port']); ?>"
                                                    data-retour-arrival-port="<?php echo htmlspecialchars($retour['arrival_port']); ?>"
                                                    data-retour-ticket-number="<?php echo htmlspecialchars($retour['ticket_number']); ?>"
                                                    data-retour-ticket-type="<?php echo htmlspecialchars($retour['ticket_type']); ?>"
                                                    data-retour-seat-number="<?php echo htmlspecialchars($retour['seat_number']); ?>"
                                                <?php endif; ?>>Voir le Ticket</button>
                                        <button class="btn edit-btn"
                                                ...
                                                data-with-baby="<?php echo htmlspecialchars($group['passenger_info']['with_baby']); ?>"
                                                data-departure-date="<?php echo htmlspecialchars($main_reservation['departure_date']); ?>"
                                                data-trip-id="<?php echo $main_reservation['trip_id']; ?>"
                                                data-ticket-type="<?php echo htmlspecialchars($main_reservation['ticket_type']); ?>"
                                                data-seat-number="<?php echo htmlspecialchars($main_reservation['seat_number']); ?>"
                                                data-payment-status="<?php echo htmlspecialchars($group['passenger_info']['payment_status']); ?>"
                                                data-modification="<?php echo htmlspecialchars($group['passenger_info']['modification'] ?? ''); ?>"
                                                data-penalty="<?php echo htmlspecialchars($group['passenger_info']['penalty'] ?? ''); ?>">Modifier</button>
                                        <form method="POST" style="display: inline;" onsubmit="return confirm('Voulez-vous vraiment supprimer cette réservation ?');">
                                            <input type="hidden" name="delete_reservation" value="<?php echo $main_reservation['id']; ?>">
                                            <button type="submit" class="btn delete-btn">Supprimer</button>
                                        </form>
                                        <?php if ($group['passenger_info']['payment_status'] !== 'Annulé'): ?>
                                            <form method="POST" style="display: inline;" onsubmit="return confirm('Voulez-vous vraiment annuler cette réservation ?');">
                                                <input type="hidden" name="cancel_reservation" value="<?php echo $main_reservation['id']; ?>">
                                ...
                                <?php if ($retour && $aller): ?>
                                    <tr>
                                        ...
                                        <td></td>
                                        <td>Retour</td>
                                        <td><?php echo htmlspecialchars($retour['departure_date'] . ' ' . $retour['departure_time']); ?></td>
                                        <td><?php echo htmlspecialchars($retour['departure_port'] . ' → ' . $retour['arrival_port']); ?></td>
                                        <td><?php echo htmlspecialchars($group['passenger_info']['reference_number']); ?></td>
                                        <td><?php echo htmlspecialchars($retour['ticket_number']); ?></td>
                                        <td><?php echo htmlspecialchars($retour['ticket_type']); ?></td>
                                        <td><?php echo htmlspecialchars($retour['seat_number']); ?></td>
                                        <td><?php echo htmlspecialchars($group['passenger_info']['payment_status']); ?></td>
                                        <td><?php echo htmlspecialchars(number_format($retour['tariff'], 2)); ?></td>
                                        <td><?php echo htmlspecialchars(number_format($retour['port_fee'], 2)); ?></td>
                                        <td><?php echo htmlspecialchars(number_format($retour['tax'], 2)); ?></td>
                                        <td><?php echo htmlspecialchars(number_format($retour['total_amount'], 2)); ?></td>
                                        <td><?php echo htmlspecialchars($group['passenger_info']['modification'] ?? 'Aucune'); ?></td>
                                        <td><?php echo htmlspecialchars($group['passenger_info']['penalty'] ?? 'Aucune'); ?></td>
                                        <td></td>
                                    </tr>
                ...
                <div class="ticket-modal-content">
                    ...
                    <div class="ticket">
                        ...
                        <div class="section section-2a">
                            ...
                            <p><strong>Trajet Aller :</strong> <span id="route"></span><br><span class="translation">Outbound Route</span></p>
                        </div>
                        ...
                        <div class="section section-4a">
                            <p><strong>Trajet Retour :</strong> <span id="retour_route"></span><br><span class="translation">Return Route</span></p>
                            <p><strong>Date Retour :</strong> <span id="retour_departure_date"></span><br><span class="translation">Return Date</span></p>
                            <p><strong>Numéro de Billet Retour :</strong> <span id="retour_ticket_number"></span><br><span class="translation">Return Ticket Number</span></p>
                        </div>
                        <div class="section section-4b">
                            <p><strong>Type de Billet Retour :</strong> <span id="retour_ticket_type"></span><br><span class="translation">Return Ticket Type</span></p>
                            <p><strong>Place Retour :</strong> <span id="retour_seat_number"></span><br><span class="translation">Return Seat Number</span></p>
                            <p><strong>Départ Retour :</strong> <span id="retour_departure_time"></span><br><span class="translation">Return Departure</span></p>
                        </div>
                        <div class="section section-5a">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            viewTicketButtons.forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    // Gérer le trajet retour
                    if (button.dataset.retourId) {
                        document.getElementById('retour_route').textContent = `${button.dataset.retourDeparturePort} -> ${button.dataset.retourArrivalPort}`;
                        document.getElementById('retour_departure_date').textContent = button.dataset.retourDepartureDate;
                        document.getElementById('retour_ticket_number').textContent = button.dataset.retourTicketNumber;
                        document.getElementById('retour_ticket_type').textContent = button.dataset.retourTicketType;
                        document.getElementById('retour_seat_number').textContent = button.dataset.retourSeatNumber;
                        document.getElementById('retour_departure_time').textContent = `${button.dataset.retourDepartureDate} ${button.dataset.retourDepartureTime}`;
                    } else {
                        document.getElementById('retour_route').textContent = 'N/A';
                        document.getElementById('retour_departure_date').textContent = 'N/A';
                        document.getElementById('retour_ticket_number').textContent = 'N/A';
                        document.getElementById('retour_ticket_type').textContent = 'N/A';
                        document.getElementById('retour_seat_number').textContent = 'N/A';
                        document.getElementById('retour_departure_time').textContent = 'N/A';
                    }
            ...
            window.exportToPDF = function() {
                ...
                const ticketData = {
                    ...
                    retour_route: document.getElementById('retour_route').textContent,
                    retour_departure_date: document.getElementById('retour_departure_date').textContent,
                    retour_ticket_number: document.getElementById('retour_ticket_number').textContent,
                    retour_ticket_type: document.getElementById('retour_ticket_type').textContent,
                    retour_seat_number: document.getElementById('retour_seat_number').textContent,
                    retour_departure_time: document.getElementById('retour_departure_time').textContent,
                ...
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Trajet Aller: ' + ticketData.route, margin, y);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Outbound Route', margin, y + 2);
                y += 5;
                let yRight = 16;
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                ...
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Trajet Retour: ' + ticketData.retour_route, margin, y);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Route', margin, y + 2);
                y += 5;
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Date Retour: ' + ticketData.retour_departure_date, margin, y);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Date', margin, y + 2);
                y += 5;
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Numéro Billet Retour: ' + ticketData.retour_ticket_number, margin, y);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Ticket Number', margin, y + 2);
                y += 5;
                yRight = Math.max(yRight, y - 15);
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Type Billet Retour: ' + ticketData.retour_ticket_type, margin + colWidth + 4, yRight);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Ticket Type', margin + colWidth + 4, yRight + 2);
                yRight += 5;
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Place Retour: ' + ticketData.retour_seat_number, margin + colWidth + 4, yRight);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Seat Number', margin + colWidth + 4, yRight + 2);
                yRight += 5;
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
                doc.setFont('helvetica', 'bold');
                doc.text('Départ Retour: ' + ticketData.retour_departure_time, margin + colWidth + 4, yRight);
                doc.setFontSize(7);
                doc.setTextColor(102, 102, 102);
                doc.setFont('helvetica', 'italic');
                doc.text('Return Departure', margin + colWidth + 4, yRight + 2);
                yRight += 5;
                y = Math.max(y, yRight);
                doc.setFontSize(9);
                doc.setTextColor(0, 0, 0);
